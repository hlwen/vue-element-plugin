  ...
  <div class="queryForm">
    <el-form :inline="true" :model="query" ref="queryform" label-width="100px" class="demo-query">
      <el-form-item v-for="item in formItems" :label="item.label + ':'" :prop="item.key">
        ...
        <el-date-picker v-if="['date','daterange','datetimerange'].indexOf(item.type) >-1" v-model="query[item.key]" :type="item.type" placeholder="请选择"></el-date-picker>
        ...
        <remote-select v-else-if="item.type=='remoteSelect'" v-model="query[item.key]" :prop="item.prop" :url="item.url"></remote-select>
